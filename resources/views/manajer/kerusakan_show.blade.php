            ...
            <div class="bg-ketiga rounded-lg shadow-md p-8 mt-8 max-w-4xl mx-auto">
                <div class="flex">
                    ...
                    <div class="sm:w-1/3 mt-5">
                        @if (!empty($kerusakan->foto_kerusakan[0]))
                            <img id="mainImage" src="{{ asset('storage/' . $kerusakan->foto_kerusakan[0]) }}" alt="Foto Kerusakan" class="rounded-lg shadow-lg w-full h-full object-cover">
                        @else
                    ...
                    <div class="md:w-2/3 md:ml-8 mt-5">
                        ...
                        @if ($kerusakan->status === 'disetujui_asisten' || $kerusakan->status === 'disetujui')
                ...
                <div class="flex justify-start mt-8 space-x-2 overflow-x-auto">
                    @foreach ($kerusakan->foto_kerusakan as $foto)
                        <img src="{{ asset('storage/' . $foto) }}" alt="Foto Kerusakan" class="w-20 h-20 object-cover rounded-lg shadow-lg cursor-pointer" onclick="swapImage(this)">
                    @endforeach
                </div>
                ...
                @elseif (auth()->user()->role === 'manajer' && !in_array($kerusakan->status, ['disetujui', 'ditolak']))
                    <div class="flex flex-col items-center mt-20 space-y-4">
                        <p class="font-bold text-lg">Apakah Anda menyetujui laporan ini?</p>
                        <div class="flex space-x-4 mt-5">
