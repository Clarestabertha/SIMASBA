                ...
                <div class="w-full max-w-4xl px-4 sm:px-8 py-4 overflow-x-auto">
                    ...
                    <div class="inline-block min-w-full shadow rounded-lg overflow-hidden mt-5">
                        <table class="min-w-full leading-normal">
                            ...
                            <tbody>
    @php
        $dataSedang = $kerusakan->filter(fn($item) => $item->status === 'sedang diproses' || $item->status === 'disetujui_asisten'|| $item->status === 'ditolak_asisten' );
        $dataLainnya = $kerusakan->filter(fn($item) => !in_array($item->status, ['sedang diproses', 'disetujui_asisten','ditolak_asisten']));
    @endphp
