                    ...
                    <div class="inline-block min-w-full shadow rounded-lg overflow-hidden mt-5">
                        <table class="min-w-full leading-normal">
                            ...
                            <tbody id="user-table-body">
                                @php
                                    $dataSedang = $kerusakan->filter(function ($item) {
                                        return $item->status === 'sedang diproses';
                                    });
                                    $dataLainnya = $kerusakan->filter(function ($item) {
                                    ...
                                    <tr class="bg-white">
                                        ...
                                        <td class="px-5 py-5 border border-gray-200 text-sm text-center">
                                            <div class="flex justify-center space-x-2">
                                                <a href="{{ route('kerusakan.asmen.show', $k->id_kerusakan) }}" class="mx-2">
                    ...
                    <div class="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
                        ...
                        <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                            ...
                            <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                                ...
                                @else
                                <a href="{{ $kerusakan->previousPageUrl() }}" class="relative inline-flex items-center rounded-l-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-500 hover:bg-gray-50">
                                    <span class="sr-only">Previous</span>
                                    <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                    <path fill-rule="evenodd" d="M12.293 15.707a1 1 0 01-1.414 0L5.586 10l5.293-5.707a1 1 0 011.414 1.414L8.414 10l3.879 3.879a1 1 0 010 1.414z" clip-rule="evenodd" />
                                    </svg>
                                ...
                                <!-- Page number links -->
                                @for ($i = 1; $i <= $kerusakan->lastPage(); $i++)
                                <a href="{{ $kerusakan->url($i) }}" class="{{ $kerusakan->currentPage() == $i ? 'bg-primary text-white' : 'bg-white text-gray-500' }} relative inline-flex items-center border border-gray-300 px-4 py-2 text-sm font-medium hover:bg-gray-50">
                                    {{ $i }}
                                </a>
                                @endfor

                                @if ($kerusakan->hasMorePages())
                                <a href="{{ $kerusakan->nextPageUrl() }}" class="relative inline-flex items-center rounded-r-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-500 hover:bg-gray-50">
